...
# .venv/bin/python create_rectangle_geopackage.py --bbox "2'589'985/1'204'604 2'590'254/1'204'600 2'590'254/1'204'400 2'589'985/1'204'400" --interval 0.5
global resample
...
def export_to_image_pdf(interval, resample=None):
    ...
    # Add WMS image as background with 50% opacity
    image = Image.open(BytesIO(response.read()))
    ax.imshow(image, extent=(min_x, max_x, min_y, max_y), origin='upper', alpha=0.5)

    # Step 6: Plot the Contours
        ...
        if row['geometry'] is not None:
            ...
            if row['line_width'] == 3.5:
                ...
                if line.length < 100:
                    ...
                    ax.text(point.x, point.y, f"{int(row['ELEV'])} m", fontsize=16, ha='center', color='blue')
                else:
                    num_labels = int(line.length // 60)
                    for i in range(num_labels):
                        point = line.interpolate(i * 60)
                        x_label, y_label = point.x, point.y
                        next_point = line.interpolate(i * 60 + 1) if (i * 60 + 1) < line.length else point
                        angle = np.degrees(np.arctan2(next_point.y - y_label, next_point.x - x_label))
                        ax.text(x_label, y_label, f"{int(row['ELEV'])} m", fontsize=16, ha='center', color='blue', rotation=angle, rotation_mode='anchor')

    # Step 7: Plot the Rectangle Overlay
    ...
    # Step 8: Set Axis Limits and Labels
    ...
    # Add a Scale Bar and Text Label
    ax.add_artist(ScaleBar(1))
    display_value = 0.5 if resample is None else resample
    ax.text(0.001, 0.001, f'Äquidistanz: {interval}m DEM: {display_value}m', ha='left', va='bottom', transform=ax.transAxes, fontsize=14, color='black')
    ...
    # Export the contours and rectangle to DXF
    dxf_output_path = output_path + '.dxf'
    command = ['ogr2ogr', '-f', 'DXF', dxf_output_path, gpkg_path, layer_name]
    try:
        subprocess.run(command, check=True)
    ...
    command = ['ogr2ogr', '-f', 'DXF', rect_output_path, rectangle_path, "rectangle_layer"]
    try:
        subprocess.run(command, check=True)
